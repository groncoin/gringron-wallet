...
//! Main for building the binary of a GrinGron Reference Wallet
...
use gringron_wallet_impls::HTTPNodeClient;
use std::env;
use std::path::PathBuf;

use gringron_wallet::cmd;
...
pub fn info_strings() -> (String, String) {
	(
		format!(
			"This is GrinGron Wallet version {}{}, built for {} by {}.",
...
fn real_main() -> i32 {
	let yml = load_yaml!("gringron-wallet.yml");
	...
	let mut config = match config::initial_setup_wallet(&chain_type, current_dir, create_path) {
		...
		Err(e) => match e {
			ConfigError::PathNotFoundError(m) => {
				println!("Wallet configuration not found at {}. (Run `gringron-wallet init` to create a new wallet)", m);
				return 0;
			}
			m => {
				println!("Unable to load wallet configuration: {} (Run `gringron-wallet init` to create a new wallet)", m);
				return 0;
			}
		},
	};
