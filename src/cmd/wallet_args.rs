...
use crate::config::GRINGRON_WALLET_DIR;
...
use gringron_core::core::amount_to_hr_string;
use gringron_keychain as keychain;
use gringron_wallet_api::Owner;
use gringron_wallet_config::{config_file_exists, TorConfig, WalletConfig};
use gringron_wallet_controller::{command, Error};
use gringron_wallet_impls::{DefaultLCProvider, DefaultWalletImpl};
use gringron_wallet_libwallet::{self, Slate, SlatepackAddress, SlatepackArmor};
use gringron_wallet_libwallet::{IssueInvoiceTxArgs, NodeClient, WalletInst, WalletLCProvider};
	...
	K: keychain::Keychain + 'static,
{
	...
	match wallet_args.subcommand() {
		...
		_ => {
			let msg = format!("Unknown wallet command, use 'gringron-wallet help' for details");
