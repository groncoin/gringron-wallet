...
use gringron_wallet_config::{TorConfig, WalletConfig};
use gringron_wallet_controller::command::GlobalArgs;
use gringron_wallet_controller::Error;
use gringron_wallet_impls::DefaultWalletImpl;
use gringron_wallet_libwallet::{NodeClient, StatusMessage, WalletInst, WalletLCProvider};
...
const COLORED_PROMPT: &'static str = "\x1b[36mgringron-wallet>\x1b[0m ";
const PROMPT: &'static str = "gringron-wallet> ";
	...
	K: keychain::Keychain + 'static,
{
	...
	let yml = load_yaml!("../bin/gringron-wallet.yml");
	...
	loop {
		match reader.readline(PROMPT) {
			Ok(command) => {
				...
				// Just add 'gringron-wallet' to each command behind the scenes
				// so we don't need to maintain a separate definition file
				let augmented_command = format!("gringron-wallet {}", command);
				let args =
