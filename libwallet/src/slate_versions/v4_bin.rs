...
use crate::gringron_core::core::transaction::{FeeFields, OutputFeatures};
use crate::gringron_core::ser as gringron_ser;
use crate::gringron_core::ser::{Readable, Reader, Writeable, Writer};
use crate::gringron_keychain::BlindingFactor;
use crate::gringron_util::secp::key::PublicKey;
use crate::gringron_util::secp::pedersen::{Commitment, RangeProof};
use crate::gringron_util::secp::Signature;
...
impl Writeable for SlateStateV4 {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), gringron_ser::Error> {
		let b = match self {
...
impl Readable for SlateStateV4 {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateStateV4, gringron_ser::Error> {
...
impl Readable for UuidWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<UuidWrap, gringron_ser::Error> {
...
impl Readable for SlateOptFields {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptFields, gringron_ser::Error> {
...
impl Readable for SigsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<SigsWrap, gringron_ser::Error> {
...
impl Readable for SlateOptStructs {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateOptStructs, gringron_ser::Error> {
		let status = reader.read_u8()?;
...
impl Readable for ComsWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ComsWrap, gringron_ser::Error> {
...
impl Readable for ProofWrap {
	fn read<R: Reader>(reader: &mut R) -> Result<ProofWrap, gringron_ser::Error> {
...
impl serde::Serialize for SlateV4Bin {
	...
	{
		let mut vec = vec![];
		gringron_ser::serialize(&mut vec, gringron_ser::ProtocolVersion(4), self)
		...
		D: serde::Deserializer<'de>,
	{
		...
		impl<'de> serde::de::Visitor<'de> for SlateV4BinVisitor {
				...
				E: serde::de::Error,
			{
				...
				let s = gringron_ser::deserialize(
					&mut reader,
					gringron_ser::ProtocolVersion(4),
					gringron_ser::DeserializationMode::default(),
				)
...
impl Readable for SlateV4Bin {
	fn read<R: Reader>(reader: &mut R) -> Result<SlateV4Bin, gringron_ser::Error> {
...
fn slate_v4_serialize_deserialize() {
	use crate::gringron_util::from_hex;
	use crate::gringron_util::secp::key::PublicKey;
	use crate::Slate;
	use gringron_core::global::{set_local_chain_type, ChainTypes};
	use gringron_keychain::{ExtKeychain, Keychain, SwitchCommitmentType};
	...
	let mut vec = Vec::new();
	let _ = gringron_ser::serialize_default(&mut vec, &v4_bin).expect("serialization failed");
	let b4_bin_2: SlateV4Bin = gringron_ser::deserialize_default(&mut &vec[..]).unwrap();
