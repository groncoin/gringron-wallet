// Copyright 2021 The GrinGron Develope;
//
...
use crate::gringron_core::core::hash::Hashed;
use crate::gringron_core::core::{Output, OutputFeatures, Transaction};
use crate::gringron_core::libtx::proof;
use crate::gringron_keychain::ViewKey;
use crate::gringron_util::secp::key::SecretKey;
use crate::gringron_util::Mutex;
use crate::gringron_util::ToHex;
use crate::util::{OnionV3Address, OnionV3AddressError};

use crate::api_impl::owner_updater::StatusMessage;
use crate::gringron_keychain::{BlindingFactor, Identifier, Keychain, SwitchCommitmentType};
	...
	K: Keychain + 'a,
{
	if !update_wallet_state(
		...
		return Err(Error::TransactionCancellationError(
			"Can't contact running GrinGron node. Not Cancelling.",
		));
	}
	wallet_lock!(wallet_inst, w);
	let parent_key_id = w.parent_key_id();
...
/// get stored tx
/// crashes if stored tx has total fees exceeding 2^40 nanogringron
