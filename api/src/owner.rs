...
use gringron_wallet_libwallet::RetrieveTxQueryArgs;
use uuid::Uuid;
...
use gringron_wallet_util::OnionV3Address;
	...
	K: Keychain,
{
	...
	///
	/// Each method will call the [`WalletBackend`](../gringron_wallet_libwallet/types/trait.WalletBackend.html)'s
	/// [`open_with_credentials`](../gringron_wallet_libwallet/types/trait.WalletBackend.html#tymethod.open_with_credentials)
	/// (initialising a keychain with the master seed,) perform its operation, then close the keychain
	/// with a call to [`close`](../gringron_wallet_libwallet/types/trait.WalletBackend.html#tymethod.close)
	///
	/// # Arguments
	...
	/// updates
	/// [`WalletBackend`](../gringron_wallet_libwallet/types/trait.WalletBackend.html) trait.
	///
	/// # Returns
	...
	/// # Returns
	/// * Result Containing:
	/// * A Vector of [`AcctPathMapping`](../gringron_wallet_libwallet/types/struct.AcctPathMapping.html) data
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Remarks
	///
	...
	/// ```
	/// # gringron_wallet_api::doctest_helper_setup_doc_env!(wallet, wallet_config);
	///
	/// let api_owner = Owner::new(wallet.clone(), None);
	///
	...
	/// # Returns
	/// * Result Containing:
	/// * A [Keychain Identifier](../gringron_keychain/struct.Identifier.html) for the new path
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Remarks
	///
	...
	/// a node (via the [`NodeClient`](../gringron_wallet_libwallet/types/trait.NodeClient.html)
	...
	/// [OutputCommitMapping](../gringron_wallet_libwallet/types/struct.OutputCommitMapping.html)
	/// of which each element is a mapping between the wallet's internal
	/// [OutputData](../gringron_wallet_libwallet/types/struct.Output.html)
	...
	/// Returns a list of [Transaction Log Entries](../gringron_wallet_libwallet/types/struct.TxLogEntry.html)
	...
	/// the given [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html) uuid.
	/// * `tx_query_args` - If provided, use advanced query arguments as documented in
	/// (../gringron_wallet_libwallet/types.struct.RetrieveTxQueryArgs.html). If either
	...
	/// [TxLogEntries](../gringron_wallet_libwallet/types/struct.TxLogEntry.html)
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	///
	/// # Returns
	/// * (`bool`, [`WalletInfo`](../gringron_wallet_libwallet/types/struct.WalletInfo.html)) - A tuple:
	/// * The first `bool` element indicates whether the data was successfully
	/// refreshed from the node (note this may be false even if the `refresh_from_node`
	/// argument was set to `true`.
	/// * The second element contains the Summary [`WalletInfo`](../gringron_wallet_libwallet/types/struct.WalletInfo.html)
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	/// [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html) object containing
	...
	/// to the recipient).
	///
	/// If the `send_args` [`InitTxSendArgs`](../gringron_wallet_libwallet/types/struct.InitTxSendArgs.html),
	/// of the [`args`](../gringron_wallet_libwallet/types/struct.InitTxArgs.html), field is Some, this
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `args` - [`InitTxArgs`](../gringron_wallet_libwallet/types/struct.InitTxArgs.html),
	/// transaction initialization arguments. See struct documentation for further detail.
	///
	/// # Returns
	/// * a result containing:
	/// * The transaction [Slate](../gringron_wallet_libwallet/slate/struct.Slate.html),
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `args` - [`IssueInvoiceTxArgs`](../gringron_wallet_libwallet/types/struct.IssueInvoiceTxArgs.html),
	/// invoice transaction initialization arguments. See struct documentation for further detail.
	///
	/// # Returns
	/// * ``Ok([`slate`](../gringron_wallet_libwallet/slate/struct.Slate.html))` if successful,
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `slate` - The transaction [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html). The
	...
	/// [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html),
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `slate` - The transaction [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html). All
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `tx` - A completed [`Transaction`](../gringron_core/core/transaction/struct.Transaction.html),
	/// typically the `tx` field in the transaction [`Slate`](../gringron_wallet_libwallet/slate/struct.Slate.html).
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `tx_id` - If present, cancel by the [`TxLogEntry`](../gringron_wallet_libwallet/types/struct.TxLogEntry.html) id
	/// for the transaction.
	///
	...
	///
	/// # Returns
	/// * Ok(Some([Slate](../gringron_wallet_libwallet/slate/struct.Slate.html)) containing the stored
	...
	/// * [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	/// ```
	...
	/// [TxLogEntry](../gringron_wallet_libwallet/types/struct.TxLogEntry.html) is created.
	///
	/// # Arguments
	///
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	...
	///
	/// # Returns
	/// * Ok with a  [`NodeHeightResult`](../gringron_wallet_libwallet/types/struct.NodeHeightResult.html)
	...
	/// `gringron-wallet.toml` file and the `wallet_data` directory that contains the wallet
	...
	///
	/// 1) The current directory, from which `gringron-wallet` or the main process was run, if it
	/// contains a `gringron-wallet.toml` file.
	/// 2) ~/.gringron/<chaintype>/ otherwise
	///
	/// # Arguments
	///
	/// * None
	...
	/// Create a `gringron-wallet.toml` configuration file in the top-level directory for the
	/// specified chain type.
	/// A custom [`WalletConfig`](../gringron_wallet_config/types/struct.WalletConfig.html)
	/// and/or gringron `LoggingConfig` may optionally be provided, otherwise defaults will be used.
	///
	...
	/// # gringron_wallet_api::doctest_helper_setup_doc_env!(wallet, wallet_config);
	///
	/// use gringron_core::global::ChainTypes;
	///
	/// let dir = "path/to/wallet/dir";
	///
	/// # let dir = tempdir().map_err(|e| format!("{:#?}", e)).unwrap();
	...
	/// path-related values in the optional configuration structs will be ignored.
	///
	/// The wallet files must not already exist, and ~The `gringron-wallet.toml` file must exist
	...
	///
	/// # Returns
	/// * Ok with a Vec of [`StatusMessage`](../gringron_wallet_libwallet/api_impl/owner_updater/enum.StatusMessage.html)
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	///
	/// # Returns
	/// * Ok with a [Slate](../gringron_wallet_libwallet/slate/struct.Slate.html) if successful
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	///
	/// # Returns
	/// * Ok with a [Slatepack](../gringron_wallet_libwallet/slatepack/types/struct.Slatepack.html) if successful
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered.
	///
	/// # Example
	/// Set up as in [`new`](struct.Owner.html#method.new) method above.
	...
	/// Returns a single, exportable [PaymentProof](../gringron_wallet_libwallet/api_impl/types/struct.PaymentProof.html)
	...
	///
	/// # Returns
	/// * Ok([PaymentProof](../gringron_wallet_libwallet/api_impl/types/struct.PaymentProof.html)) if successful
	/// * or [`libwallet::Error`](../gringron_wallet_libwallet/struct.Error.html) if an error is encountered
	...
	/// Verifies a [PaymentProof](../gringron_wallet_libwallet/api_impl/types/struct.PaymentProof.html)
	...
	/// * `keychain_mask` - Wallet secret mask to XOR against the stored wallet seed before using, if
	/// being used.
	/// * `proof` A [PaymentProof](../gringron_wallet_libwallet/api_impl/types/struct.PaymentProof.html))
	///
	/// # Returns
