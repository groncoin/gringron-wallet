...
pub const WALLET_CONFIG_FILE_NAME: &str = "gringron-wallet.toml";
const WALLET_LOG_FILE_NAME: &str = "gringron-wallet.log";
const GRINGRON_HOME: &str = ".gringron";
/// Wallet data directory
pub const GRINGRON_WALLET_DIR: &str = "wallet_data";
/// Node API secret
pub const API_SECRET_FILE_NAME: &str = ".foreign_api_secret";
/// Owner API secret
pub const OWNER_API_SECRET_FILE_NAME: &str = ".owner_api_secret";

fn get_gringron_path(
	chain_type: &global::ChainTypes,
	create_path: bool,
) -> Result<PathBuf, ConfigError> {
	// Check if gringron dir exists
	let mut gringron_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	gringron_path.push(GRINGRON_HOME);
	gringron_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !gringron_path.exists() && create_path {
		fs::create_dir_all(gringron_path.clone())?;
	}

	if !gringron_path.exists() {
		Err(ConfigError::PathNotFoundError(String::from(
			gringron_path.to_str().unwrap(),
		)))
	} else {
		Ok(gringron_path)
	}
...
fn check_api_secret_file(
	...
	let gringron_path = match data_path {
		Some(p) => p,
		None => get_gringron_path(chain_type, false)?,
	};
	let mut api_secret_path = gringron_path;
...
pub fn initial_setup_wallet(
	...
	// Use config file if current directory if it exists, .gringron home otherwise
	let (path, config) = if let Some(p) = check_config_current_dir(WALLET_CONFIG_FILE_NAME) {
		...
		let gringron_path = match data_path {
			Some(p) => p,
			None => get_gringron_path(chain_type, create_path)?,
		};

		// Get path to default config file
		let mut config_path = gringron_path.clone();
		...
		match config_path.clone().exists() {
			false => {
				...
				default_config.update_paths(&gringron_path);
				(gringron_path, default_config)
			}
			true => {
...
impl GlobalWalletConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalWalletConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Testnet => {
				defaults.api_listen_port = 15635;
				defaults.check_node_api_http_addr = "http://127.0.0.1:15633".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_listen_port = 25635;
				defaults.check_node_api_http_addr = "http://127.0.0.1:25633".to_owned();
			}
			_ => {}
		}
		defaults_conf
	...
	pub fn update_paths(&mut self, wallet_home: &PathBuf) {
		...
		wallet_path.push(GRINGRON_WALLET_DIR);
