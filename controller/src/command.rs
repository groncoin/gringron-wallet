...
//! GrinGron wallet command-line function implementations
	...
	controller::owner_single_use(None, keychain_mask, Some(owner_api), |api, m| {
		if args.estimate_selection_strategies {
			let strategies = vec!["smallest", "all"]
				...
				.collect::<Result<Vec<_>, gringron_wallet_libwallet::Error>>()?;
			...
			slate = match result {
				Ok(s) => {
					info!(
						"Tx created: {} gringron to {} (strategy '{}')",
	...
	controller::owner_single_use(None, keychain_mask, Some(owner_api), |api, m| {
		if args.estimate_selection_strategies {
			...
			slate = match result {
				Ok(s) => {
					info!(
						"Invoice processed: {} gringron (strategy '{}')",
