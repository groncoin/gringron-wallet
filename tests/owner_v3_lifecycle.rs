...
use gringron_wallet_api::{ECDHPubkey, JsonId};
...
use gringron_wallet_libwallet::{InitTxArgs, Slate, SlateVersion, VersionedSlate};
use serde_json;

use gringron_util::Mutex;
...
#[test]
fn owner_v3_lifecycle() -> Result<(), gringron_wallet_controller::Error> {
	...
	let (chain, wallet2, mask2_i) = {
		...
		let arg_vec = vec!["gringron-wallet", "owner_api", "-l", "43420", "--run_foreign"];
	...
	// 2) get the top level directory, should default to ~/.gringron/auto
	...
	println!("RES 4: {:?}", res);
	assert!(res.is_ok());
	let pb = PathBuf::from(format!("{}/wallet1/gringron-wallet.toml", test_dir));
	assert!(pb.exists());
	...
	//15) Ask wallet 2 for some gringrons
	let req = serde_json::json!({
		"jsonrpc": "2.0",
		"id": 1,
		...
		"params": {
			...
			"args": {
				"amount": "6000000000",
				"message": "geez a block of gringrons",
				"dest_acct_name": null,
