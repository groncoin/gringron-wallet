...
extern crate gringron_apiwallet as apiwallet;
extern crate gringron_libwallet as libwallet;
extern crate gringron_refwallet as wallet;
extern crate gringron_wallet_config as wallet_config;
#[macro_use]
extern crate log;
...
use gringron_p2p as p2p;
use gringron_servers as servers;
...
fn simulate_block_propagation() {
	...
	let test_name_dir = "gringron-prop";
	framework::clean_all_output(test_name_dir);
...
fn simulate_full_sync() {
	...
	let test_name_dir = "gringron-sync";
	framework::clean_all_output(test_name_dir);

	let s1 = servers::Server::new(framework::config(1000, "gringron-sync", 1000)).unwrap();
	...
	let s2 = servers::Server::new(framework::config(1001, "gringron-sync", 1000)).unwrap();
...
fn simulate_fast_sync() {
	...
	let test_name_dir = "gringron-fast";
	framework::clean_all_output(test_name_dir);

	// start s1 and mine enough blocks to get beyond the fast sync horizon
	let s1 = servers::Server::new(framework::config(2000, "gringron-fast", 2000)).unwrap();
	let stop = Arc::new(Mutex::new(StopState::new()));
	s1.start_test_miner(None, stop.clone());
	...
	let mut conf = config(2001, "gringron-fast", 2000);
...
fn simulate_long_fork() {
	...
	let test_name_dir = "gringron-long-fork";
	framework::clean_all_output(test_name_dir);
...
fn long_fork_test_preparation() -> Vec<servers::Server> {
	...
	let mut conf = framework::config(2100, "gringron-long-fork", 2100);
	conf.archive_mode = Some(false);
	...
	for i in 1..6 {
		let mut conf = config(2100 + i, "gringron-long-fork", 2100);
		conf.archive_mode = Some(false);
		conf.api_secret_path = None;
...
/// Intended to replicate https://github.com/groncoin/gringron/issues/1325
#[ignore]
#[test]
